...
|---|---|---|---|
| `is_dcm` | `dcmdump` | `-M +L +Qn <file>` | Probes any file to decide if it is DICOM. `+Qn` quiet, `+L` long tags, `-M` load short. Run on both a DICOM and a non-DICOM input. |
